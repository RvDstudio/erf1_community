    ...
    (set, get) => ({
      ...
      add: (item: WishlistItem) => {
        if (!get().items.find((i: WishlistItem) => i.id === item.id)) {
          set((state: WishlistState) => ({ items: [...state.items, item] } as Partial<WishlistState>))
        }
      },
      remove: (id: string) => {
        set((state: WishlistState) => ({ items: state.items.filter((i: WishlistItem) => i.id !== id) } as Partial<WishlistState>))
      },
